...
export default function ArbeitsnachweisPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Extrahiere Fehlermeldung aus verschiedenen Error-Typen
      let errorMessage = "Fehler beim Speichern des Arbeitsnachweises"
      if (err instanceof Error) {
        errorMessage = err.message
      } else if (typeof err === 'object' && err !== null && 'message' in err) {
        // Supabase/PostgrestError haben eine message-Property
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      if (isHeicFile(originalFile)) {
        ...
        try {
          ...
          let errorMsg = "HEIC-Konvertierung fehlgeschlagen"
          if (conversionError instanceof Error) {
            errorMsg = conversionError.message
          } else if (typeof conversionError === 'object' && conversionError !== null && 'message' in conversionError) {
            errorMsg = String((conversionError as { message: string }).message)
          }
          setError(errorMsg)
