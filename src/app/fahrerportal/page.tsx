...
import { signIn, canAccessFahrerportal } from "@/lib/api"
...
export default function FahrerportalLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Prüfe Fahrerportal-Zugang (erlaubt Fahrer, Admin/GF mit Fahrer-Datensatz)
      const accessResult = await canAccessFahrerportal(user.id)

      if (!accessResult.canAccess) {
        // Zeige benutzerfreundliche Fehlermeldung
        setError(accessResult.reason || "Kein Fahrerportal-Zugang")
        setIsLoading(false)
        return
      }
