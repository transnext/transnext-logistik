 ...
 *
 * Wichtig: Verfügbarkeit gilt nur für Werktage (Mo-Fr), Feiertage sind ausgenommen.
 */

import type { Alert, AlertSeverity } from "./alerts-api"
import { isWeekdayMoFr, isHoliday } from "./holidays"
 ...
 *
 * Wichtig: Nur Mo-Fr Werktage prüfen, Wochenenden und Feiertage ignorieren.
 */
function checkVerfuegbarOhneTour(
  ...
  // NUR Mo-Fr Werktage, keine Wochenenden oder Feiertage
  const gueltigeVerfuegbarkeiten = driverAvailability.filter(v =>
    v.is_available === true &&
    v.date < today && // Nur vergangene Tage
    isWeekdayMoFr(v.date) && // Nur Mo-Fr
    !isHoliday(v.date) && // Keine Feiertage
